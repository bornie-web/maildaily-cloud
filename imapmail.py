"""Generic read-only IMAP adapters. TLS, EXAMINE and BODY.PEEK only; never STORE/EXPUNGE.

Supported providers live in the PROVIDERS table; each gets isolated encrypted
storage (prefix '<id>.') and its own /v1/<id>/ routes, schedule and watermark.
...
PROVIDERS = {
    'netease': {
        'name': '163', 'host': 'imap.163.com', 'port': 993, 'imap_id': True,
        'pattern': r'[a-z0-9_.+-]+@163\.com',
        'hint': '请填写完整的 @163.com 邮箱和无空格的客户端授权码。',
        'web': 'https://mail.163.com/', 'label': '163 收件箱 · 只读',
    },
    'netease126': {
        'name': '126', 'host': 'imap.126.com', 'port': 993, 'imap_id': True,
        'pattern': r'[a-z0-9_.+-]+@126\.com',
        'hint': '请填写完整的 @126.com 邮箱和无空格的客户端授权码。',
        'web': 'https://mail.126.com/', 'label': '126 收件箱 · 只读',
    },
    'yeah': {
        'name': 'yeah', 'host': 'imap.yeah.net', 'port': 993, 'imap_id': True,
        'pattern': r'[a-z0-9_.+-]+@yeah\.net',
        'hint': '请填写完整的 @yeah.net 邮箱和无空格的客户端授权码。',
        'web': 'https://mail.yeah.net/', 'label': 'yeah 收件箱 · 只读',
    },
    'qq': {
        'name': 'QQ 邮箱', 'host': 'imap.qq.com', 'port': 993, 'imap_id': False,
        'pattern': r'[a-z0-9_.+-]+@(qq|foxmail)\.com',
        'hint': '请填写完整的 QQ 邮箱（@qq.com 或 @foxmail.com）和无空格的 IMAP 授权码。',
        'web': 'https://mail.qq.com/', 'label': 'QQ 收件箱 · 只读',
    },
    'icloud': {
        'name': 'iCloud', 'host': 'imap.mail.me.com', 'port': 993, 'imap_id': False,
        'pattern': r'[a-z0-9_.+-]+@(icloud|me|mac)\.com',
        'hint': '请填写完整的 iCloud 邮箱（@icloud.com/@me.com/@mac.com）和 App 专用密码。',
        'web': 'https://www.icloud.com/mail', 'label': 'iCloud 收件箱 · 只读',
    },
}
...
@contextmanager
def inbox(cfg, account):
    name = cfg['name']
    client = None
    try:
        client = imaplib.IMAP4_SSL(cfg['host'], cfg['port'],
                                  ssl_context=ssl.create_default_context(), timeout=15)
        check(client.login(account['email'], account['code'])[0],
              f'{name} 登录失败，请检查授权码（不是登录密码）。')
        if cfg['imap_id'] and b'ID' in client.capabilities:
            # ID extension is required by NetEase-family accounts. Identify truthfully.
            imaplib.Commands.setdefault('ID', ('AUTH', 'SELECTED'))
            check(client._simple_command('ID', '("name" "MailDaily" "version" "0.4" "vendor" "MailDaily")')[0],
                  f'{name} 客户端识别失败，请检查网页端客户端访问设置。')
        check(client.select('INBOX', readonly=True)[0],
              f'{name} 拒绝只读访问，请在网页邮箱确认 IMAP 已开启、授权码有效及客户端安全设置。')
        response = client.response('UIDVALIDITY')[1]
        if not response or not response[0] or not response[0].isdigit():
            raise HTTPException(502, f'{name} 未返回稳定的邮箱标识，本次未保存。')
        yield client, int(response[0])
    except HTTPException:
        raise
    except imaplib.IMAP4.error:
        raise HTTPException(502, f'{name} 连接被拒绝。请确认使用授权码/App 专用密码，并在网页端检查 IMAP 开关和安全提醒。') from None
    except (OSError, ValueError, UnicodeError):
        raise HTTPException(502, f'无法安全连接 {name}，请检查云端网络后重试；未修改原邮件。') from None
    finally:
        if client:
...
def message_id(cfg, validity, uid):
    if not (0 < validity <= 0xffffffff and 0 < uid <= 0xffffffff):
        raise HTTPException(502, f"{cfg['name']} 邮件标识无效。")
...
def read_message(cfg, client, validity, uid, budget):
    name = cfg['name']
    check_deadline(cfg, budget)
    status, rows = client.uid('FETCH', str(uid), '(UID INTERNALDATE RFC822.SIZE)')
    check(status, f'{name} 邮件信息读取失败，本期未保存。')
    ...
    if not size or not date:
        raise HTTPException(502, f'{name} 邮件已移动或信息不完整，请重试。')
    n = int(size[1])
    ...
    check(status, f'{name} 邮件正文读取失败，本期未保存。')
    chunks = [x[1] for x in rows if isinstance(x, tuple) and isinstance(x[1], bytes)]
    raw = b''.join(chunks)
    if len(raw) != n or len(raw) > MAX_MESSAGE:
        raise HTTPException(502, f'{name} 邮件下载不完整，本期未保存。')
    budget['bytes'] += len(raw)
    parsed = BytesParser(policy=policy.default).parsebytes(raw)
    return {'id': message_id(cfg, validity, uid), 'internalDate': str(stamp), 'labelIds': [],
            'payload': mime_payload(parsed)}


def check_deadline(cfg, budget):
    if time.monotonic() > budget['deadline']:
        raise HTTPException(504, f"{cfg['name']} 读取超时，旧简报和进度不变，请稍后重试。")


def fetch_mail(cfg, account, start, end):
    ...
    with inbox(cfg, account) as (client, validity):
        status, rows = client.uid('SEARCH', None, 'SINCE', day(start, -1), 'BEFORE', day(end, 2))
        check(status, f"{cfg['name']} 邮件搜索失败，本期未保存。")
        ids = sorted({int(x) for x in (rows[0] or b'').split()})
        if len(ids) > MAX_CANDIDATES:
            raise HTTPException(422, f"{cfg['name']} 搜索候选超过200封，未保存部分结果；此初版暂不适合大批量邮箱。")
        result = []
        for uid in ids:
            mail = read_message(cfg, client, validity, uid, budget)
            if mail: result.append(mail)
        return result


def fetch_attachment(cfg, account, mid, part_id):
    ...
    with inbox(cfg, account) as (client, validity):
        if validity != int(mid[:8], 16):
            raise HTTPException(409, f"{cfg['name']} 邮箱标识已变化，请重新整理后查看附件。")
        mail = read_message(cfg, client, validity, int(mid[8:], 16), {'bytes': 0, 'deadline': time.monotonic()+60})
        ...
        if len(raw) > 10*1024*1024: raise HTTPException(413, f"附件超过10MB，请在{cfg['name']}网页邮箱打开。")
...
def register_imap(app, store, auth, pid):
    cfg = PROVIDERS[pid]
    name = cfg['name']
    prefix = pid + '.'
    lock = asyncio.Lock()
    ...
    def account():
        value = get('account')
        if not value: raise HTTPException(409, f'请先连接{name}邮箱。')
        return value
    def clear():
    ...
    class Schedule(BaseModel):
        ...
        enabled: bool
    @app.get(f'/v1/{pid}/state', dependencies=[Depends(auth)])
    async def state():
        value = get('account', {})
        return {'connected': bool(value), 'account': value.get('email',''), 'readOnly': True, 'provider': pid,
                'schedule': get('schedule', DEFAULT_SCHEDULE), 'lastError': get('error'), 'busy': lock.locked()}
    @app.post(f'/v1/{pid}/connect', dependencies=[Depends(auth)])
    async def connect(value: Login):
        email = value.email.strip().lower(); code = value.code.get_secret_value().strip()
        if not re.fullmatch(cfg['pattern'], email) or not code or re.search(r'[\s\x00-\x1f\x7f]', code):
            raise HTTPException(422, cfg['hint'])
        async with lock:
            saved = get('account')
            if saved and saved['email'] != email: raise HTTPException(409, f'请先断开当前{name}邮箱，再换另一个账号。')
            value = {'email': email, 'code': code}
            def verify():
                with inbox(cfg, value): pass
            await asyncio.to_thread(verify)
            store.put(prefix+'account', value)
        return {'connected': True}
    @app.delete(f'/v1/{pid}/account', dependencies=[Depends(auth)])
    async def disconnect():
        async with lock: clear()
        return {'cleared': True, 'revocationRequired': True}
    @app.get(f'/v1/{pid}/digests', dependencies=[Depends(auth)])
    async def digests():
        return {'account': get('account',{}).get('email',''), 'scheduleLabel': cfg['label'], 'digests': get('digests',[])}
    async def sync(key=None):
        if lock.locked(): raise HTTPException(409, f'{name}正在连接或整理，请稍后刷新。')
        async with lock:
            saved = account(); end = int(time.time()*1000); start = get('watermark', end-86400000)
            try:
                messages = await asyncio.to_thread(fetch_mail, cfg, saved, start, end)
                ...
                for item in digest['items']:
                    item['links'] = [{'label': item['title'], 'url': cfg['web']}]
                ...
                detail = e.detail if isinstance(e, HTTPException) else f'{name}整理失败，旧简报与进度不变。'
                store.put(prefix+'error', detail)
                raise HTTPException(502, detail) from None
        return digest
    @app.post(f'/v1/{pid}/sync', dependencies=[Depends(auth)])
    async def manual_sync(): return await sync()
    @app.put(f'/v1/{pid}/schedule', dependencies=[Depends(auth)])
    async def schedule(value: Schedule):
        async with lock: store.put(prefix+'schedule', {**value.model_dump(), 'timezone': 'Asia/Shanghai'})
        return {'saved': True}
    @app.get(f'/v1/{pid}/messages/{{mid}}/attachments/{{part_id}}', dependencies=[Depends(auth)])
    async def attachment(mid: str, part_id: str):
        async with lock:
            ...
            if not known: raise HTTPException(404, f'附件不在当前{name}简报中。')
            return await asyncio.to_thread(fetch_attachment, cfg, saved, mid, part_id)
    async def tick():
...
def register_imap_providers(app, store, auth):
    """Register every provider in PROVIDERS. Returns {pid: tick}."""
    return {pid: register_imap(app, store, auth, pid) for pid in PROVIDERS}
